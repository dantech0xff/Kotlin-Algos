...
class RadixSortTest : FunSpec({
    val sorter = RadixSort()
    ...
    test("sorts list with duplicates") {
        ...
        val result = sorter.sort(input)
        result.isSorted() shouldBe true
        result shouldBe listOf(1, 1, 2, 3, 4, 5, 5, 6, 9)
    }
    ...
    test("sorts negative numbers") {
        val input = listOf(-5, 3, -1, 0, 2, -3)
        val result = sorter.sort(input)
        result.isSorted() shouldBe true
        result shouldBe listOf(-5, -3, -1, 0, 2, 3)
    }
    ...
    test("sorts large range values") {
        val input = listOf(1, 1000000, 500, 99999)
        val result = sorter.sort(input)
        result.isSorted() shouldBe true
        result shouldBe listOf(1, 500, 99999, 1000000)
    }
